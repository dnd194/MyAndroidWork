...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_main2);  //여기 레이아웃에서
        etName = findViewById(R.id.etName);           //이 레이아웃에 있는 id를 찾는거임
        ...
        btnStartTwo.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("msg","하위");

                //이름 ,나이 --> Person 에 담은 뒤 Intent 에 실어 보내기

                Person p = new Person(    //person은 String , int 를 받음
                  etName.getText().toString(), Integer.parseInt(etAge.getText().toString())
                );

                intent.putExtra("person",p);  // person이란 이름으로 객체 p를 보냄
        ...
        btnFinish.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                finish(); //액티비티 종료   ---> 화면이 없어진거지 앱이 종료된 것이 아니다
            }
        });

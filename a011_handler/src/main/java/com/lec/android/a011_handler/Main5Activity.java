...
public class Main5Activity extends AppCompatActivity {
    ...
    // AsyncTask<Params, Progress, Result>       제네릭임
//      Params: doItBackground 에서 사용할 변수 타입
//      Progress: onProgress 에서 사용할 변수의 타입
//      Result : onPostExecute 에서 사용할 변수의 타입
    class BackgroundTask extends AsyncTask<Integer, Integer, Integer> {

        //백그라운드 작업 시작하기전에 호출해야 할 것
        @Override
        ...
        //백그라운드 작업  '''반드시 구현''' 해야함
        @Override
        protected Integer doInBackground(Integer... integers) {    //처음 시작할 때 doInBackground
            //   ...  ->  가변매개변수  , integers = integer[] (배열) 이다

            for(backValue1 = 0; backValue1<integers[0]; backValue1++){
                if(backValue1 % 10 == 0 ){
                    publishProgress(backValue1);
                    //progress 상태를 update 뽑아냄 --> onProgressUpdate 호출되고 매개변수 값 전달됨
                }
                try {
                    Thread.sleep(100);
            ...
            return backValue1;  //이 return 값은 onPostExecute에 넘어간다.
            // ※ doInBackground() 에서 시간이 많이 걸리는 '통신' 작업이나 복잡한 연산 작업등을 (비동기로)수행케 해야 한다.
        }
        //백그라운드 작업도중 여러번 호출 가능, 진행상황 업데이트, 중간중간 UI 업데이트
        @Override
        protected void onProgressUpdate(Integer... values) {
            Log.d("myapp", "Progress : " + values[0] + "%");
            //publishProgress (i) 가 보낸 값 values
            super.onProgressUpdate(values);
            tvBackValue1.setText("onProgressUpdate : " + values[0]);  // handler 없이도 main UI 접근가능
            //가장 중요한 것
        }
        //doInBackground() 완료 되면 호출
        @Override
        protected void onPostExecute(Integer integer) {  //doinbackground의 return 값을 매개변수로 받는다.
            //doinbackground는 Integer형을 반환함
            Log.d("myapp","Result :" +integer);
            super.onPostExecute(integer);
            tvBackValue1.setText("onPostExecute : "+integer);   // handler 없이도 main UI 접근가능
        }
    }//end AsyncTask

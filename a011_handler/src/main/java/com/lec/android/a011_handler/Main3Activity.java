...
public class Main3Activity extends AppCompatActivity {
    ...
    void doUpload(int n){
        Toast.makeText(getApplicationContext(), n +": 업로드 완료했습니다",Toast.LENGTH_SHORT).show();
    }
    ...
    */
    public void mOnClick2(View v){
        new AlertDialog.Builder(this).setTitle("질문 2")
                .setMessage("업러드 할껍니까??")
                .setPositiveButton("네", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        mhandler.postDelayed(new Runnable() {   // 매개변수를 2개 받음 runnable 과 시간딜레이
                            @Override
                            public void run() {
                                doUpload(2);
                            }
    ...
    public void mOnClick3(View v){
        new AlertDialog.Builder(this).setTitle("질문 3")
                .setMessage("업러드 할껍니까??")
                .setPositiveButton("네", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
    ...
    //# 4. Timer, TimerChecker
    public void mOnClick4(View v){
        new AlertDialog.Builder(this).setTitle("질문 4")
                .setMessage("업러드 할껍니까??")
                .setPositiveButton("네", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        Timer timer = new Timer();
                        TimerTask task = new TimerTask() {  //알아서 익명클래스가 생성됨
                            @Override
                            public void run() {
                                //예약할 작업 내용 기술
                                mhandler.sendEmptyMessage(4);  //여기서 딜레이주는게 아니라 아래에서 줌
                            }
                        };
                        timer.schedule(task,3000);  //schedule 
                    }
                })
                .setNegativeButton("아니요",null)
                .show();

...
public class MainActivity extends AppCompatActivity {
    ...
    public void mOnClick(View v){   //이 동작은 mainActivity에서 하고 있음
        ...
        thread1.setDaemon(true);   //메인스레드와 종료 동기화   setDaemon
        thread1.start();  // 작업 스레드 시작

        BackRunnable runnable = new BackRunnable();
        Thread thread2 = new Thread(runnable);  //runnable 을 implement 하는 쓰레드
    ...
    class BackThread extends Thread{
        ...
        public void run() {
            while(true){
                ...
                //작업쓰레드에서 메인 UI 를 직접건드릴 수 없다
//                tvBackValue1.setText(""+backValue1);   CalledFromWrongThreadException
            }
        }
    }//end BackThread

    //2. runnable을 implement 하는 작업스레드
    class BackRunnable implements Runnable{
        @Override
        public void run() {
            while(true){
                backValue2 +=2 ;  // 작업스레드 값 2씩 증가

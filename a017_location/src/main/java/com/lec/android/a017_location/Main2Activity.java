...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //LocationListener 객체
        mLocationListener = new MyListener();  //생성

        //토글버튼 세팅
        btnLocate.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (btnLocate.isChecked()) {     //버튼이 눌리면
                    //위치정보 수신 시작
                    // 주어진 위치 제공자가 기억하는 가장 최근의 위치정보 획득, 없으면 null return
                    try {
                        Location location = manager.getLastKnownLocation(LocationManager.GPS_PROVIDER);
                        if(location !=null ){
                            tvResult.setText("최근 위치 : "+locationInfo(location));
                        }
                        manager.requestLocationUpdates(
                               LocationManager.GPS_PROVIDER, //등록할 위치 제공자
                                100,            // 통지사이의 최소 시간 간격
                                1,           // 통지사이의 최소 변경거리 (단위 : M(미터))

...
public class Main3Activity extends AppCompatActivity {
    //이미지 경로 알아내기
    //픽셀 폰 : 이미지보기 -> 하단의 info 버튼
    //삼성 폰 : 갤러리 이미지에서 롱클릭 후 상세정보..
    //제조사마다 모델마다 다를수가있음
    private String imgPath = "/storage/emulated/0/DCIM/Camera/face03.png";

    ImageView ivPhoto;
    TextView tvPath;
    Button btnAction;

    private final int PERMISSION_REQUEST_CODE = 101;  //권한코드 요청값
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //권한을 회득하지 않은 상태에서만 요청
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (ContextCompat.checkSelfPermission(this, Manifest.permission.READ_EXTERNAL_STORAGE)
                    != PackageManager.PERMISSION_GRANTED) {
                //권한 요청 : 비동기로 진행됨.
                ActivityCompat.requestPermissions(this, PERMISSIONS,  //요청할 권한들들
                        PERMISSION_REQUEST_CODE);  //권한요청 코드
        ...
        switch (requestCode) {
            case PERMISSION_REQUEST_CODE:
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    //사용자가 권한 승인함
                    drawPhoto();
                    Log.d("myapp", "권한 획득 결과\n\t" + Arrays.toString(permissions) + " : " + Arrays.toString(permissions));

...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ll = findViewById(R.id.ll);  //레이아웃 매칭
...
// 메뉴버튼을 눌렀을 때 보여줄 menu 에 대해서 정의
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.menu_main1, menu); //만든 메뉴객체(menu_main)을 menu 로 올림
        Log.d("myapp", "onCreateOptionMenu - 최초 메뉴키를 눌렸을때 호출됨 ");

        //#2 심지어 MenuItem을 동적으로 추가가능하다!
        // 이때 add 메서드를 사용, MenuItem 리턴
        ...
        return true;  //false를 리턴하며 메뉴는 보여지지 않는다.
    }
...
// '옵션메뉴'가 화면에 보여질때마다 호출되는 메소드
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        Log.d("myapp","onPrepareOptionsMenu - 옵션 메뉴가 화면에 보여질때마다 호출됨");
        return true; //false를 리턴하며 메뉴는 보여지지 않는다.
    }
...
// '옵션메뉴의 아이템'이 선택(클릭) 되었을때 호출되는 메소드
    ...
    public void showInfo(MenuItem item){
        ...
        Log.d("myapp",msg);
        Toast.makeText(getApplicationContext(),title + "메뉴클릭", Toast.LENGTH_SHORT).show();
   }
}//end activity

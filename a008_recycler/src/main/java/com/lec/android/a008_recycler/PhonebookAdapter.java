...
public class PhonebookAdapter extends RecyclerView.Adapter<PhonebookAdapter.ViewHolder> {  //static 클래스라서 . 사용 가능
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        //그렇게 만들어진 View를 return 한다.
        View itemView = inf.inflate(R.layout.item, parent, false);

        //위에서 만들어진 새로운 View를 ViewHolder에 담아서 return 한다.
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Phonebook item = items.get(position);     //arraylist의 함수 get
        holder.setItem(item);
    }
    ...
    public int getItemCount() {
        return items.size();  //arraylist에 담겨져있는 size를 return
    }

    // nested class (static inner) 로 ViewHolder 클래스 정의
    static class ViewHolder extends RecyclerView.ViewHolder{   //반드시 생성자가 필요해서 타이핑하면 빨간줄이 생김
        //빨간줄에 마우스 대고 때리면됨
        ...
        public ViewHolder(@NonNull View itemView) {      //생성자 필수

            //item 레이아웃의 View 객체가 전달됨
            super(itemView);
            //위에 매개변수로 받는 View 에서 찾음
            //View 객체 가져오기
            ...
            //switch 리스터 장착  ==> 전화번호와 이메일 숨기기/ 보이기
            ...
            btnDelItem.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    //position의 정보가 필요함 -> 어디서 가져올거야??  ->  꼼수를 쓸거야 -> adapter 생성자 위에 static으로 만들거임
                    //adapter 로부터 데이터 삭제도 진행되어야함
                    adapter.removeItem(getAdapterPosition()); //==> 데이터 삭제
                    //데이터를 지우고 어뎁터에도 반영을 해줘야함 (데이터변경 내역 ) 그래야지 정삭적으로 동작함 ********
...
//                    Toast.makeText(v.getContext(),"position"+ position, Toast.LENGTH_SHORT).show();
                    ...
                    //액티비티가 아닌곳에서 객체를 끄집어 낼때 v.getContext()
